...
class FastTextEmbedder(TokenEmbedder):
    def __init__(
        self,
        model: Optional[Union[str, gensim.models.FastText]] = None,
        min_count: int = 1,
        epochs: int = 10,
        **optional_params: Any,
    ) -> None:
        ...
        if isinstance(model, str):
            model = cast(gensim.models.FastText, gensim.models.FastText.load(model))

        self.model = model or gensim.models.FastText(
            min_count=min_count,
            **optional_params,
        )
        self.epochs = epochs
    ...
    def fit(self, X: List[List[str]], y: Any = None) -> FastTextEmbedder:
